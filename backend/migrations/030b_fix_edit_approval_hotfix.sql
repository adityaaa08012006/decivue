-- Migration 030b: Hotfix for Edit Approval (Correct Column Names)
-- Fixes the resolve_edit_request function to use correct column names
...
BEGIN
  ...
  IF p_approved THEN
    ...
    -- Build a single UPDATE statement with all changes
...
DO $$
BEGIN
  RAISE NOTICE '✓ Migration 030b applied: resolve_edit_request() fixed with correct column names';
  RAISE NOTICE '  - Uses lifecycle (not lifecycle_stage)';
  RAISE NOTICE '  - Includes all required columns (health_signal, organization_id)';
  RAISE NOTICE '  - Consolidated UPDATE statement for better performance';
END $$;
