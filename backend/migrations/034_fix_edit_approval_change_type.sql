-- Migration 034: Fix resolve_edit_request to use correct change_type
-- Changes 'edited' to 'edit_approved' to match constraint in migration 033
...
BEGIN
  ...
  IF p_approved THEN
    ...
    -- Apply all field changes to the decision in one UPDATE
    UPDATE decisions
    SET 
    ...
    IF v_proposed_changes ? 'assumptionsToUnlink' THEN
      ...
      FOR v_assumption_id IN 
        SELECT jsonb_array_elements_text(v_assumptions_to_unlink)::uuid
      LOOP
        -- Get assumption description for version tracking
        SELECT description INTO v_assumption_desc
        FROM assumptions
        WHERE id = v_assumption_id;
        
        -- Unlink the assumption
        DELETE FROM decision_assumptions
        WHERE decision_id = v_audit.decision_id
        AND assumption_id = v_assumption_id;
        
        -- Track the relation change
        PERFORM track_relation_change(
          v_audit.decision_id,
          'assumption',
          v_assumption_id,
          'unlinked',
          v_assumption_desc,
          p_reviewer_id,
          'Edit approval: ' || v_audit.justification
        );
    ...
    IF v_proposed_changes ? 'assumptionsToLink' THEN
      ...
      FOR v_assumption_id IN 
        SELECT jsonb_array_elements_text(v_assumptions_to_link)::uuid
      LOOP
        -- Get assumption description for version tracking
        SELECT description INTO v_assumption_desc
        FROM assumptions
        ...
        -- Link the assumption (only if not already linked)
    ...
    -- Create version history entry with 'edit_approved' change_type (FIXED)
    ...
    SELECT 
      ...
      d.lifecycle,
      d.health_signal,
      'edit_approved',  -- FIXED: Changed from 'edited' to 'edit_approved'
...
-- Migration complete
COMMENT ON FUNCTION resolve_edit_request IS 'Resolves edit approval requests with support for field changes and assumption linking/unlinking. Fixed to use edit_approved change_type.';
